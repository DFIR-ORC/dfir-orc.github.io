...
Compiling the source code for DFIR ORC yields three executables: ``OrcCapsule.exe``, ``DFIR-Orc_x86.exe`` and ``DFIR-Orc_x64.exe``.

In a second step, architecture specific binaries are embedded into ``OrcCapsule.exe`` which is then renamed as ``DFIR-ORC.exe``. At runrime, ``DFIR-ORC.exe`` detect the host environment and automatically execute the appropriate version.

The binary ``DFIR-ORC.exe`` is still an **unconfigured binary**. It contains the :doc:`embedded specific tools <embedded_tool_suite>` developed as part of this project.
...
.. code:: bat
    ...
    DFIR-Orc_x64.exe ToolEmbed /embed=c:\path\to\your\config\Embed.xml /out=DFIR-Orc.exe
...
.. code:: bat
    ...
    DFIR-Orc.exe NTFSInfo /out=%temp%\test.csv c:\

:doc:`GetThis <GetThis>` collects the system software hive with:

.. code:: bat

    DFIR-Orc.exe GetThis /nolimits /sample=SOFTWARE /out=%temp%\hive.7z c:\
...
As seen `above <#dfir-orc-configuration-process>`_, the first code to run when a configured binary runs is called the **Capsule**.
As 64-bit platform can run 32-bit code but not the other way around.
...
On 32-bit systems, the configured binary is "native" to the architecture. Thus, the Capsule (here ``DFIR-Orc.exe``) will extract the embedded DFIR-ORC_x86.exe and execute it with the ``WolfLauncher`` command as shown in the figure below.
...
On 64-bit systems the extraction of ``DFIR-Orc_x64.exe`` is required.
It is followed by the execution of ``DFIR-Orc_x64.exe WolfLauncher``.
Depending on what is specified in the :doc:`WolfLauncher configuration file <wolf_config>`, it will then launch embedded tools and/or external tools to proceed with the data collection.

.. image:: _static/orc_execution_x64.svg

DFIR ORC can look up and use resources from its parent and grandparent processes (i.e. the Capsule and/or WolfLauncher) without having to extract them.
